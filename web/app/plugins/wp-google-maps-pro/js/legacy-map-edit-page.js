        ...
        var wpgmza_table_length;
			...
			jQuery(function($) {
				if(WPGMZA.settings.engine == "google-maps" && window.google) {
					...
					for(var poly_id in WPGM_PathData)
					{
						
						// TODO: Probs need to parse polygon latLngs here
						
						WPGM_Path[poly_id] = new google.maps.Polygon(
						
							jQuery.extend(
								{path: WPGM_PathData[poly_id]}, 
					...
					for(var polyline_id in WPGM_PathLineData)
					{
						// TODO: Probs need to parse polyline latLngs here
						
						WPGM_PathLine[polyline_id] = new google.maps.Polyline(
						
							jQuery.extend(
								{path: WPGM_PathLineData[polyline_id]},
					...
					for(var dataset_id in wpgmza_legacy_map_edit_page_vars.heatmaps)
					{
						...
						heatmap[dataset_id] = new google.maps.visualization.HeatmapLayer({
							data: WPGM_PathLineData[dataset_id]
						});              	
					}
				}
			});
            ...
            this.map.setOptions({styles: wpgmza_legacy_map_edit_page_vars.theme_data});
			...
			for(var dataset_id in wpgmza_legacy_map_edit_page_vars.heatmaps)
			{
				...
				var heatmap = heatmap[heatmap_id];
				
				heatmap.setMap(this.map.googleMap);
				heatmap.set("opacity", options.opacity);
				heatmap.set("radius", options.radius);
				heatmap.set("gradient", options.gradient);
			}
			...
			if(window.google)
			{
				if(wpgmza_legacy_map_edit_page_vars.bicycle_layer)
				{
					var bikeLayer = new google.maps.BicyclingLayer();
					bikeLayer.setMap(this.map.googleMap);
				}
				
				if(wpgmza_legacy_map_edit_page_vars.traffic_layer)
				{
					var trafficLayer = new google.maps.TrafficLayer();
					trafficLayer.setMap(this.map.googleMap);
				}
				
				if(wpgmza_legacy_map_edit_page_vars.transport_layer)
				{
