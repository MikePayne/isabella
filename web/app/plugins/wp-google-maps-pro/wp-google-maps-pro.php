...
function wpgmaps_admin_javascript_pro() {
    ...
    if (is_admin() && isset($_GET['page']) && isset($_GET['action']) && $_GET['page'] == 'wp-google-maps-menu' && $_GET['action'] == "edit_marker") { wpgmaps_admin_edit_marker_javascript(); }
		...
		foreach($total_dataset_array as $dataset_id) {
			...
			$datasetoptions = wpgmza_b_return_dataset_options($poly_id);
			$datasetoptions = maybe_unserialize($polyoptions->options);
			
			$dataset_array = wpgmza_b_return_dataset_array($dataset_id);
			
			if (isset($dataset_options['heatmap_opacity']))
				$opacity = floatval($dataset_options['heatmap_opacity']);
			else
				$opacity = floatval(0.6);
			
			if (isset($dataset_options['heatmap_gradient']))
				$gradient = stripslashes(html_entity_decode($dataset_options['heatmap_gradient']));
			else
				$gradient = false;
			
			if (isset($dataset_options['heatmap_radius']))
				$radius = intval($dataset_options['heatmap_radius']);
			else
				$radius = intval(20);
